...
public class shoulderCamera : BaseCameraController 
{	
	void  Awake ()
	{
	...
	void  Apply ( Transform dummyTarget ,   Vector3 dummyCenter  ){
		...
		//Shoulder camera distance operates in discrete steps because we were forced to implement
		//it with regular buttons. Creating a simple parameter mapped from -1 to 1 and changing the
		//value based on GetButton will allow you to create a simple virtual axis. When you get around to it :)
		if(Input.GetButton("LeftShoulder1") && !Input.GetButton("RightShoulder1"))
			vee = -0.1f;
		else if(Input.GetButton("RightShoulder1") && !Input.GetButton("LeftShoulder1"))
			vee = 0.1f;
		else
			vee = 0;

		
		eych = Input.GetAxis("Z-Axis/Shoulders");
